...
final class GitOptionsPanelController extends OptionsPanelController implements ActionListener {
    ...
    private Boolean validateFields() {
        ...
        String execpath = panel.executablePathTextField.getText();
        if (!GitModuleConfig.getDefault().isExecPathValid(execpath)) {
            JOptionPane.showMessageDialog(null,
                                          NbBundle.getMessage(GitPanel.class, "MSG_WARN_EXEC_PATH_TEXT"), // NOI18N
                                          NbBundle.getMessage(GitPanel.class, "MSG_WARN_FIELD_TITLE"), // NOI18N
                                          JOptionPane.WARNING_MESSAGE);
            return false;
        }
    ...
    private void onExecPathBrowseClick() {
        ...
        JFileChooser fileChooser = new AccessibleJFileChooser(NbBundle.getMessage(GitOptionsPanelController.class, "ACSD_BrowseFolder"), oldFile);   // NOI18N
        fileChooser.setDialogTitle(NbBundle.getMessage(GitOptionsPanelController.class, "Browse_title"));                                            // NOI18N
        fileChooser.setMultiSelectionEnabled(false);
        FileFilter[] old = fileChooser.getChoosableFileFilters();
        for (int i = 0; i < old.length; i++) {
            FileFilter fileFilter = old[i];
