...
public class ExceptionHandler {
    ...
    public final static int EX_ACTION_CANCELED_BY_USER = 1;
    ...
    static final String ACTION_CANCELED_BY_USER = ""; // TODO: initialize from strings

    private final GitException exception;
    private final int exceptionMask;
    
    public ExceptionHandler(GitException exception) {
    ...
    protected GitException getException() {
        return exception;
    }
    ...
    public static boolean isNoRepository(String msg) {
        msg = msg.toLowerCase();
        return msg.indexOf("abort: There is no Git repository here") > -1;                                   // NOI18N
    }
    ...
    private static boolean isAlreadyTracked(String msg) {
        return msg.indexOf(" already tracked!") > -1;                          // NOI18N
    }

    private static boolean isNotTracked(String msg) {
        return msg.indexOf(" not tracked!") > -1;                          // NOI18N
    }

    private static boolean isCannotReadCommitMsg(String msg) {
        return msg.indexOf("abort: can't read commit message") > -1;                          // NOI18N
    }

    public static boolean isUnableExecuteCmd(String msg) {
        return msg.indexOf("unable to execute Git command") > -1;                          // NOI18N
    }
   ...
   /** Analyzes exception and notifies user. **/
    ...
    public void notifyException(boolean notCancelled) {
        if (!notCancelled) {
            cancelledAction();
            return;
        }
    ...
    public static String parseExceptionMessage(GitException exception) {
        String msg = exception.getMessage();
        int idx = msg.lastIndexOf("git: "); // NOI18N
        if (idx > -1) {
